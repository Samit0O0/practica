...
<template>
  ...
  <main id="reportes">
    <section
      class="fixed bg-black bg-opacity-30 w-full h-full z-50 grid place-content-center place-items-center top-0 left-0 cursor-pointer"
      @click="(e) => hidden(e)" ref="popup">
      <img class="h-4/5 rounded-lg fade" src="@/assets/msg.jpeg" alt="" v-if="popupStatus == 0">

      <article class="bg-white dark:bg-slate-800 rounded-lg px-6 py-8 ring-1 ring-slate-900/5 shadow-xl w-96 fade"
        v-else-if="popupStatus > 1">
        <h3 class="text-slate-900 dark:text-white text-base font-medium capitalize mx-auto p-10">Ingrese la hora del
          voto:</h3>
        <input type="datetime-local" name="" id="">
        <div v-if="popupStatus == 2">
        ...
        <button class=" bg-[#ECA008] hover:bg-[#010c41] text-white font-bold py-2 px-10 rounded-3xl mx-auto block my-5"
          @click="() => alerta('Enviado', 'se ha enviado el registro', 'success')">Registrar</button>
      </article>
    ...
    <form @submit.prevent="search">
      ...
      <article id="busqueda">
        <select class="w-20 bg-white" name="tipopersona" @change="insertValue" disabled>
    ...
    <section class="grid grid-cols-2 h-3.5" v-else>
      <article class="w-full bg-[#010c41] rounded-r-full grid">
        <h2 class="place-self-center text-white text-3xl px-10 py-10">Ingrese su numero de cedula para registrar el
          voto.</h2>
      </article>
